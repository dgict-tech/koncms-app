...
const RevenueAnalytics: React.FC = () => {
  ...
  // per-video revenue map (id -> revenue number)
  ...
  // DATE RANGE CALCULATOR (stable)
  const getDateRange = React.useCallback((filter: typeof dateRange) => {
    const end = new Date();
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (saved && saved.length > 0)
        setSelectedChannel((prev) => prev ?? saved[0]);
    };
    init();
  }, []);
    ...
    const load = async () => {
      ...
      try {
        ...
        for (const vid of data || []) {
          ...
          fetchRevenueForVideo(vid.id).catch((e) => console.error(e));
        }
      } catch (error) {
        console.log(error);
      } finally {
  ...
  // Fetch revenue for a single video (similar to Videos.tsx)
  const fetchRevenueForVideo = useCallback(
    async (videoId: string) => {
      ...
      try {
        ...
        // Query total estimatedRevenue for the video (fallback period)
        ...
        setVideoRevenues((prev) => ({ ...prev, [videoId]: 0 }));
      }
    },
    [selectedChannel]
  );
  ...
  // LINE CHART DATA (simulate revenue series when real revenue not available)
  const chartData = useMemo(() => {
    ...
    // derive total revenue: prefer fetched `videoRevenues`, then explicit `video.revenue`, else derive from views
    const totalRevenue =
      selectedVideo && videoRevenues[selectedVideo.id] !== undefined
        ? Number(videoRevenues[selectedVideo.id])
        : selectedVideo.revenue !== undefined && selectedVideo.revenue !== null
        ? Number(selectedVideo.revenue)
        : Math.max(0, (Number(selectedVideo.views) || 0) * 0.002); // $0.002 per view heuristic
    ...
    if (useMonthly) {
      const labels: string[] = [];
      const spans: { year: number; month: number }[] = [];
      ...
      while (cur <= endMonth) {
        ...
        spans.push({ year: cur.getFullYear(), month: cur.getMonth() });
        cur.setMonth(cur.getMonth() + 1);
      }

      const count = labels.length || 1;
      const base = Math.floor((totalRevenue * 100) / count) / 100; // cents distributed
      const data = spans.map((_, i) => {
        const noise = rand(i) * base * 0.6;
        return Math.max(0, Math.round((base + noise) * 100) / 100);
      });

      const sum = data.reduce((a, b) => a + b, 0) || 1;
      const scaled = data.map(
        (v) => Math.round((v / sum) * totalRevenue * 100) / 100
      );
    ...
    const base = Math.floor((totalRevenue * 100) / daysCount) / 100;
  ...
  }, [selectedVideo, dateRange, getDateRange, isMobile, videoRevenues]);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Estimated revenue by video and date range
          </p>
        </div>
      </div>
      ...
      <div className="mt-10 bg-white p-6 rounded-xl shadow-lg">
        {loading ? (
          ...
          <div className="text-center text-gray-400">
            Select a video to view revenue analytics
          </div>
        ) : (
          <>
            <div className="flex flex-col sm:flex-row items-start gap-4 mb-4 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-2 text-sm text-gray-500">
                  {videoRevenues[selectedVideo.id] === undefined ? (
                    <span className="text-sm text-gray-400">
                      Loading revenue...
                    </span>
                  ) : (
                    <span className="text-green-600 font-semibold">
                      ${Number(videoRevenues[selectedVideo.id] ?? 0).toFixed(2)}{" "}
                      est.
                    </span>
                  )}
