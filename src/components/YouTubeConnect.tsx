...
const YouTubeConnect: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        const tokenResponse =
          await youtubeAuthService.exchangeCodeForTokens(code);

        if (tokenResponse?.access_token) {
          const authResult = await youtubeAuthService.authenticateToken(
          ...
          if (authResult?.data) {
            setChannels((prev) => {
              const filtered = prev.filter(
                (x) => x.channelId !== authResult.data.channelId
              );
              return [...filtered, authResult.data];
            });

            setSelectedChannel(authResult.data);
          }
        }
      } catch (err) {
  ...
  const handleConnectYouTube = async () => {
    ...
    try {
      ...
      }, 400); // allow loader animation to show
    } catch (err) {
      console.error(err);
...
h-72 sm:h-80 w-full sm:w-64 lg:w-72 border border-red-500 hover:shadow-lg transition">
          ...
          >
            {connectLoading ? (
              <>Connecting  <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div></>
            ) : (
              "+ Connect Channel"
            )}
          </button>
